...
def login_handler(url0, url1, username, password):
    ...
    #Host: 127.0.0.1:3000
    PasswordMgr1.add_password(None, url0, username, password)
    #实例一个含有密码的基础验证处理对象
...
login_handler(url0, url1, username, password)
xmlrpc = xmlrpclib.ServerProxy(url2)
b = raw_input('please input the name we need to call:')
print(xmlrpc.phone(b))

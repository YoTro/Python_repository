...
def test_v5_bids_comprehensive(keyword: str, asin: str, strategy: str = "LEGACY_FOR_SALES"):
    ...
    logger.info("--- COMPREHENSIVE V5 TEST ---")
    ...
    try:
        ...
        print("\n" + "=" * 90)
        print("ADVANCED BID ANALYSIS (Estimated Impressions)")
        print("=" * 90)
        for theme_group in bid_recs:
            ...
            for a in analyses:
                ...
                for p in points:
                    print(
                        f"${p.get('bid'):<9} | {p.get('type'):<15} | {p.get('impactMetrics', {}).get('estimatedImpressionAvg'):<20}"
                    )

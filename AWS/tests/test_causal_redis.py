...
logger.info(f"Redis data: daily_perf={len(daily_perf_raw)}, campaigns={len(campaigns)}, "
...
_CHANGE_PRIORITY = {
    "BID_AMOUNT": 3, "STATUS": 3, "BUDGET_AMOUNT": 2,
    "TARGETING": 2, "PLACEMENT": 1,
}
_NOISE_BID_PCT    = 0.03
...
logger.info(f"change_events for {ASIN}: {len(change_events)} "
...
print(f"  CausalImpact inf/nan failures : {ci_fails}  {'✅ fixed' if ci_fails == 0 else '❌ still failing'}")
print(f"  Conflicting model consensus   : {conflicts}")
print(f"  Strong evidence               : {strong}")
print(f"  Backtest: {result.get('backtest_hit_rate','N/A')}  "
      f"(n={result.get('backtest_total','?')})")
print()

for a in attrs[:10]:
    ci  = a.get("causal_impact") or {}
    its = a.get("its") or {}
    print(f"  [{a.get('changed_at','?')}] {a.get('change_type','?'):20s} "

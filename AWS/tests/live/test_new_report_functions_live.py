...
Usage:
    venv311/bin/python3 tests/live/test_new_report_functions_live.py
    venv311/bin/python3 tests/live/test_new_report_functions_live.py --days 7
    venv311/bin/python3 tests/live/test_new_report_functions_live.py --probe-budget-status
...
async def _check_change_history(store_id: str, region: str, days: int) -> bool:
    ...
    logger.info(
        "[Ads] get_change_history  2026-04-17  AD_GROUP/BID_AMOUNT  campaign=A01848952MBUG5UJOJLG1"
    )
    try:
        ...
        logger.info("  Profile-wide fetch with pagination")
...
async def probe_budget_status_events(store_id: str, region: str, days: int) -> None:
    ...
    try:
        ...
        if not events_a:
            logger.warning(
                "  No events returned — either BUDGET_STATUS is not a valid filter "
                "or no budget-status events occurred in the past %d days",
                days,
            )
    except Exception as e:
    ...
    try:
        ...
        ob = [
            e
            for e in events_b
            if (e.get("changeType") or e.get("change_type", "")).upper()
            in ("OUT_OF_BUDGET", "BUDGET_STATUS", "BUDGET")
        ]
        if ob:
...
async def main(args: argparse.Namespace) -> None:
    ...
    ok = await _check_change_history(args.store_id, args.region, args.days)

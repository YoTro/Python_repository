...
Run:
    venv311/bin/python tests/live/test_sellersprite_live.py
or via pytest:
    venv311/bin/python -m pytest tests/live/test_sellersprite_live.py -m live -v -s
"""
...
def _resolve_node_path() -> list[dict]:
...
def node_id_path() -> str:
    """First resolved node's colon-joined id path — feeds test_competing_lookup."""
    nodes = _resolve_node_path()
    assert nodes, "resolve_node_path returned empty — check token file"
    assert nodes[0].get("id"), "first node has no 'id' field"
    return nodes[0]["id"]


def test_resolve_node_path() -> None:
    nodes = _resolve_node_path()
    assert nodes, "resolve_node_path returned empty — check token file"
    assert nodes[0].get("id"), "first node has no 'id' field"
...
def _competing_lookup(node_id_path: str) -> dict:
    from src.mcp.servers.market.sellersprite.client import SellerspriteAPI
...
def test_competing_lookup(node_id_path: str) -> None:
    result = _competing_lookup(node_id_path)
    items = result.get("items") or []
...
if __name__ == "__main__":
    ...
    nodes = _resolve_node_path()
    assert nodes, "resolve_node_path returned empty — check token file"
    node_id_path_value = nodes[0]["id"]

    _competing_lookup(node_id_path_value)

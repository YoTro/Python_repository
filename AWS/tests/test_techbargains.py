...
async def test_fetch_techbargains_direct():
    ...
    try:
        ...
        pytest.fail(
            f"Failed to fetch or parse TechBargains: {e} | Response Status: {response.status_code if response else 'N/A'}"
        )
    finally:
        session.close()
    ...
    try:
        for page in range(1, max_pages + 1):
            ...
            assert response.status_code == 200, (
                f"Pagination: Expected 200 OK on page {page}, got {response.status_code}"
            )
        ...
        print(
            f"Successfully collected {len(all_deals)} deals from TechBargains across {max_pages} pages."
        )

...
bidding_strategies = list(
    {c.get("bidding_strategy") for c in campaigns if c.get("bidding_strategy")}
)
...
orders_reliability = "high" if total_orders >= 100 else "medium" if total_orders >= 30 else "low"
...
item = {
    ...
    "budget_likely_exhausted": (budget_exhaustion_pct is not None and budget_exhaustion_pct > 0.90),
...
class _MockCtx:
    def __init__(self):
        self.config = _MockConfig({"days": DAYS})

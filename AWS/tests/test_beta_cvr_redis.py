...
logger.info(
    f"[{_source}] daily_perf={len(daily_perf_raw)}, "
...
if _snap_item.get("campaign_ids"):
    ...
    campaign_ids = {str(c) for c in _snap_item["campaign_ids"]}
else:
    # Redis: derive from daily_perf (all records belong to this ASIN's campaigns)
    campaign_ids = {str(rec["campaign_id"]) for rec in daily_perf_raw if rec.get("campaign_id")}
...
print(
    f"  {'Keyword':<28} {'MT':7} {'Clicks':>6} {'Orders':>6} {'raw_cvr':>8} "
    f"{'Wilson':>8} {'Beta':>8} {'Δ%':>7} {'μ':>7}"
)
print(
    f"  {'-' * 28} {'-' * 7} {'-' * 6} {'-' * 6} {'-' * 8} {'-' * 8} {'-' * 8} {'-' * 7} {'-' * 7}"
)
...
for kw in kw_perf[:20]:
    ...
    print(
        f"  {kw['keyword_text'][:28]:<28} {mt:7} {clicks:>6} {orders:>6} {raw:>8.4f} "
...
for kw in all_zero_order_kws[:8]:
    ...
    print(
        f"  {kw['keyword_text'][:28]:<28} {mt:7} clicks={kw['total_clicks']:>4}  "
...
DAILY_BUDGET = sum(
    float(c.get("daily_budget", 0) or 0)
    for c in campaigns
    if str(c.get("campaign_id")) in campaign_ids and (c.get("state") or "").upper() == "ENABLED"
)
...
campaign_budgets = {
    cid: float(c.get("daily_budget") or 0) for cid, c in camp_meta.items() if c.get("daily_budget")
}
TARGET_ACOS = 0.35

logger.info(
    f"LP input: {len(lp_input)} keywords, budget=${DAILY_BUDGET:.2f}, avg_price=${avg_price}"
)
...
print(
    f"  {'Keyword':<30} {'MT':7} {'Clicks':>7} {'Spend':>8} {'Orders':>7} {'pess_cvr':>9} {'prior_mu':>9}"
)
...
for a in alloc[:15]:
    ...
    print(
        f"  {kw_text[:30]:<30} {mt:7} {a['optimized_clicks']:>7.1f} "
...
print(
    f"  sample_orders missing  : {missing_orders}/{len(lp_input)}  "
    f"{'✅' if missing_orders == 0 else '❌'}"
)
print(
    f"  prior_mu missing       : {missing_prior_mu}/{len(lp_input)}  "
...
if s_vals:
    ...
    print(
        f"  min={s_vals[0]:.0f}  p25={s_vals[n // 4]:.0f}  "

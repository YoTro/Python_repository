...
class TestIdentityPropagation(unittest.IsolatedAsyncioTestCase):
    ...
    def test_workflow_context_injection(self):
        ...
        ctx = WorkflowContext(
            job_id="job_001", tenant_id=req.tenant_id, user_id=req.user_id, config=req.params
        )
    ...
    async def test_agent_session_and_metadata(self, mock_pb, mock_mcp_client):
        ...
        session = session_mgr.create(
            session_id="test_sess", tenant_id="tenant_agent", user_id="user_agent"
        )
        ...
        # 注入身份元数据
        test_input["_metadata"] = {"tenant_id": session.tenant_id, "user_id": session.user_id}

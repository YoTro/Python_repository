...
async def test_product_screening_full_funnel(mock_config, mock_mcp):
    ...
    with (
        patch(
            "src.mcp.servers.amazon.extractors.profitability_search.ProfitabilitySearchExtractor.search_products",
            side_effect=side_effect_profitability,
        ),
        patch(
            "src.mcp.servers.amazon.extractors.past_month_sales.PastMonthSalesExtractor.get_batch_past_month_sales",
            return_value={
                "PASS01": 600,
                "FAIL_PRICE": 600,
                "FAIL_MARGIN": 600,
        ...
        patch(
            "src.mcp.servers.amazon.extractors.fulfillment.FulfillmentExtractor.get_fulfillment_info",
            return_value={"FulfilledBy": "Amazon"},
        ),
        patch(
            "src.mcp.servers.market.deals.client.DealHistoryClient.get_deals_for_asins",
            return_value={},
        ),
                        ...
                        text=json.dumps(
                            {
                                "status": "recalled",
                                "count": 1,
                                "findings": [{"title": "Exploding Product"}],
                            }
                        ),
                    )
                ]
        ...
        result = await workflow.execute(
            job_id="test_screening_job", params={"initial_items": initial_items}, ctx=ctx
        )

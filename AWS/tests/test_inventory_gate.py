...
logger.info(
    f"Redis: daily_perf={len(daily_perf_raw)}, campaigns={len(campaigns)}, "
...
logger.info(
    f"total_orders={total_orders:.1f}, daily_sales={daily_sales:.2f}/day, avg_price={avg_price}"
)
...
logger.info(
    f"LP OPTIMAL: {len(alloc)} keywords allocated, spend=${result['summary']['actual_spend']:.2f}"
)
...
print(
    f"  total_available={TOTAL_AVAILABLE}u  inbound_receiving={INBOUND_RECEIVING}u  "
...
for label, lead_days in scenarios:
    ...
    spend_up_camp = [
        ...
        if a["action"]
        in ("increase_budget", "enable_and_increase_budget", "enable_and_review_bids")
    ]
    ...
    print(f"\n── {label} ──")
    print(
        f"   effective_stock_days = {eff_days}  "
        f"({'GATE TRIGGERED ✅' if gate_ok else 'gate clear'})"
    )
    print(
        f"   spend-up campaign actions : {len(spend_up_camp)} total, {len(gated_camp)} gated (P2)"
    )
    print(f"   spend-up keyword actions  : {len(spend_up_kw)} total, {len(gated_kw)} gated (P2)")

    for a in gated_camp[:3]:
        p = a.get("prerequisite", {})
        print(
            f"     [CAMP {a['campaign_id'][:12]}..] action={a['action']} priority={a['priority']} "
            f"| {p.get('note', '')[:80]}"
        )
    for a in gated_kw[:3]:
        p = a.get("prerequisite", {})
        print(
            f"     [KW {a['keyword_text'][:20]}] action={a['action']} priority={a['priority']} "
            f"| {p.get('note', '')[:80]}"
        )

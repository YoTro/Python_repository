...
class TestCombinedConstraints:
    ...
    def test_infeasible_floor_relaxes_to_optimal(self):
        # min_clicks floor forces eff_cpc × min_clicks > total_budget: infeasible
        # as posed, but the solver retries with floors zeroed (over-constraint
        # recovery) rather than failing outright.
        kws = [_kw("kw|EXACT", avg_cpc=100.0, max_clicks=10, min_clicks=10)]
        r = _solve(kws, total_budget=1.0)
        # min spend = 100 × 10 = 1000 > budget 1 → infeasible with floor,
        # recovered by relaxing the floor to 0.
        assert r["status"] == "OPTIMAL"
        assert r.get("relaxed") is True

...
def api():
    ...
    with patch("src.mcp.servers.market.sellersprite.client.requests.Session") as mock_session_cls:
        ...
        with patch.object(SellerspriteAPI, "_ensure_cookies", return_value=None):
...
class TestGetCompetingLookup:
    ...
    def test_auth_is_cookie_based_not_header(self, api):
        """Auth is carried entirely by session cookies (rank-login-user etc.) —
        no separate Auth-Token header is sent; the server ignores it for v2/v3
        endpoints. See SellerspriteAPI docstring."""
        client, mock_request = api
        mock_request.return_value = _ok({"data": {"items": [], "total": 0, "page": 1, "size": 100}})

        client.session.cookies.get.return_value = "my-secret-token"
        client.get_competing_lookup(
            market="US",
            month_name="bsr_sales_monthly_202509",
            node_id_paths=["1"],
        ...
        _, kwargs = mock_request.call_args
        assert "Auth-Token" not in kwargs["headers"]

    def test_non_200_returns_empty_result(self, api):
        client, mock_request = api
        # 403 (not 500) — SERVER_ERROR is retryable and would exhaust all 3
        # attempts and raise RetryableError instead of returning gracefully.
...
class TestGetCategoryNodes:
    ...
    def test_auth_is_cookie_based_not_header(self, api):
        """Auth is carried entirely by session cookies — no Auth-Token header."""
        client, mock_request = api
        mock_request.return_value = _ok({"data": []})

        client.session.cookies.get.return_value = "secret"
        client.get_category_nodes(
            market_id=1,
            table="bsr_sales_monthly_202509",
            node_id_path="1",

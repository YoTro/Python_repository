...
def test_pricing_optimization():
    ...
    assert rec["optimal_price"] > 0
    assert rec["expected_monthly_sales"] > 0
    assert rec["expected_monthly_profit"] > 0
    assert isinstance(result["comparison_to_current"]["profit_change"], str)

...
class TestLayer3TokenBucket(unittest.TestCase):
    ...
    def test_burst_depletes(self):
        """After draining burst=5, acquire_source floors the timeout to one
        full refill cycle (+5s) regardless of the caller-supplied timeout, so
        a short requested timeout still succeeds once a token refills instead

...
Run with:
    PYTHONPATH=. pytest tests/unit/test_lp_single_kw_stress.py -v
"""
...
class TestZeroOrderExclusionGap:
    ...
    def test_beta_cvr_handles_zero_orders_via_prior(self):
        ...
        prior_mu = 0.05
        result = _beta_cvr(raw_cvr, clicks, orders, prior_mu=prior_mu)
        assert result > 0.0, "_beta_cvr should return prior-based estimate for zero-order keyword"
...
class TestC3FloorInfeasibility:
    ...
    def test_c3_alone_without_floor_never_fails(self):
        ...
        for target_acos in [0.01, 0.10, 0.30, 1.0]:
            ...
            assert r["allocation"] == [], (
                f"No clicks should be allocated to an ACOS-violating keyword; got {r['allocation']}"
            )
...
class TestCVRDeflationBoundary:
    ...
    def test_deflation_boundary_search_with_floor(self):
        ...
        assert results[0.10].get("relaxed")
    ...
    def test_deflation_floor_collapses_earlier_than_no_floor(self):
        ...
        r_fl = _solve(
            self._deflate([kw_with_floor], factor), 100.0, target_acos=0.35, avg_price=20.0
        )
        assert r_no["status"] == "OPTIMAL"
        assert not r_no.get("relaxed")  # no floor → no relaxation needed
        assert r_fl["status"] == "OPTIMAL"
        assert r_fl.get("relaxed")  # floor required relaxation
...
class TestSingleKeywordLP:
    ...
    def test_single_kw_solves_across_click_regimes(self, clicks, orders):
        ...
        kw = _lp_kw(
            ...
            avg_cpc=1.0,
            cvr=cvr,
            sample_clicks=clicks,
            prior_mu=cvr,
            max_clicks=float(clicks) * 3,
        )
    ...
    def test_single_kw_budget_sweep(self, budget_factor):
        kw = _lp_kw(
            "kw|EXACT", avg_cpc=1.0, cvr=0.10, sample_clicks=100, prior_mu=0.10, max_clicks=200.0
        )
    ...
    def test_single_kw_external_prior_shrinks_cvr(self):
        ...
        kw_ext = _lp_kw(
            "kw|EXACT", avg_cpc=1.0, cvr=raw_cvr, sample_clicks=50, prior_mu=0.05, max_clicks=100.0
        )
    ...
    @pytest.mark.parametrize(
        "avg_cpc,cvr,target_acos,avg_price,min_clicks,expect_relaxed",
        [
...
class TestOverConstrainedSmallN:
    ...
    def test_n_floored_keywords_feasible_before_breakpoint(self, n):
        ...
        """
        kws = [_lp_kw(f"kw{i}|EXACT", avg_cpc=1.0, min_clicks=3.0) for i in range(n)]
    ...
    def test_n_floored_keywords_recovers_past_breakpoint(self):
        ...
        kws = [_lp_kw(f"kw{i}|EXACT", avg_cpc=1.0, min_clicks=3.0) for i in range(7)]
        result = _solve(kws, total_budget=20.0)
        assert result["status"] == "OPTIMAL"
        assert result.get("relaxed")

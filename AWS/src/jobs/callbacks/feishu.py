...
class FeishuCallback(JobCallback):
        ...
        if interaction_type == "AUTH_QR_SCAN":
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(qr_url) as resp:
                            if resp.status == 200:
                                ...
                                logger.info(
                                    f"Download successful. Image size: {len(image_data)} bytes"
                                )
                                ...
                                logger.info(
                                    f"Temporary file created at: {temp_path}. Uploading to Feishu..."
                                )
                                ...
                                if hasattr(self.feishu, "upload_image"):
                                    ...
                                    upload_res = await asyncio.to_thread(
                                        self.feishu.upload_image, temp_path
                                    )
                                else:
                                    logger.info(
                                        "feishu.upload_image not found, falling back to upload_file..."
                                    )
                                    upload_res = await asyncio.to_thread(
                                ...
                                if upload_res.get("success") or upload_res.get("code") == 0:
                                    ...
                                    image_key = (
                                        data_obj.get("image_key")
                                        or data_obj.get("file_key")
                                        or (
                                            data_obj.get("app", {})
                                            if isinstance(data_obj.get("app"), dict)
                                            else {}
                                        ).get("image_key")
                                    )
                                    ...
                                    logger.error(
                                        f"Feishu upload failed. Code: {upload_res.get('code')}, Msg: {upload_res.get('msg')}"
                                    )
            ...
            card = {
                ...
                "header": {
                    ...
                    "title": {
                        "tag": "plain_text",
                        "content": ui_config.get("title", "🔐 需要认证"),
                    },
                },
                "elements": [
                    {
                        ...
                        "text": {
                            "tag": "lark_md",
                            "content": ui_config.get("description", "请扫描下方二维码完成登录。"),
                        },
                    }
                ],
            }
            ...
            if image_key:
                ...
                if image_key_str and image_key_str != "None":
                    ...
                    logger.warning(
                        "Extracted image_key is empty or invalid string, using fallback."
                    )
                    card["elements"].append(
                        {
                            "tag": "div",
                            "text": {
                                "tag": "lark_md",
                                "content": f"⚠️ 图片上传成功但未拿到有效 Key，请[点击此处查看二维码]({qr_url})",
                            },
                        }
                    )
            else:
                ...
                card["elements"].append(
                    {
                        ...
                        "text": {
                            "tag": "lark_md",
                            "content": f"无法显示二维码？[👉 点击此处查看微信登录二维码]({qr_url})",
                        },
                    }
                )
                ...
                [
                    {
                        ...
                        "elements": [
                            {
                                "tag": "plain_text",
                                "content": f"有效期 {data.get('expires_in', 120)} 秒，扫码后请点击下方按钮确认。",
                            }
                        ],
                    },
                    {
                        ...
                        "actions": [
                            {
                                ...
                                "text": {
                                    "tag": "plain_text",
                                    "content": ui_config.get("button_text", "我已确认扫码"),
                                },
                                "type": "primary",
                                "value": {
                                    "action": action_name,
            ...
            send_res = await asyncio.to_thread(
                self.feishu.send_raw_card, "chat_id", self.chat_id, card
            )
    ...
    async def on_complete(self, result) -> None:
        ...
        try:
            ...
            if file_path and os.path.exists(file_path):
                ...
                try:
                    ...
                    upload_res = self.feishu.upload_file(
                        file_path, file_type="stream", file_name=os.path.basename(file_path)
                    )
                ...
                if items:
                    ...
                    for key in ("response", "ad_diagnosis_llm", "result", "output"):
                        ...
                        if artifact_sent:
                            ...
                            logger.info(
                                "on_complete: no text key found after artifact delivery; using concise completion text"
                            )
                        else:
                ...
                if len(text.encode("utf-8")) > _CARD_LIMIT_BYTES:
                    try:
                        ...
                        with tempfile.NamedTemporaryFile(
                            ...
                            tmp_path = tmp.name
                        upload_res = await asyncio.to_thread(
                            self.feishu.upload_file, tmp_path, f"report{suffix}", "stream"
                        )
                        _os.unlink(tmp_path)
                        if upload_res.get("success"):
            ...
            # Create Bitable
            create_res = self.feishu.create_bitable(bitable_name, user_access_token=self.user_token)
            ...
            create_data = (
                json.loads(create_res["data"])
                if isinstance(create_res["data"], str | bytes)
                else create_res["data"]
            )
            ...
            tables_data = (
                json.loads(tables_res["items"])
                if isinstance(tables_res["items"], str | bytes)
                else tables_res["items"]
            )
            ...
            records = []
            for item in items:
                fields = {k: str(v) if v is not None else "" for k, v in item.items()}
    ...
    async def notify(self, message: str) -> None:
        try:
            await asyncio.to_thread(self.feishu.send_text_message, "chat_id", self.chat_id, message)
        except _FEISHU_ERRORS as e:

...
def _compute_cvr_prior(
    ...
    # Require at least _MIN_KWS_FOR_STRATUM contributing keywords before trusting
    # the cross-keyword weighted mean. With fewer keywords (e.g. N=1) the weighted
    # mean equals the sole keyword's own CVR, creating a circular prior in _beta_cvr
    # (prior_mu == raw_cvr → zero shrinkage).
    global_mu = (_wmean(all_pairs) if len(all_pairs) >= _MIN_KWS_FOR_STRATUM else None) or 0.02
...
def _build_lp_input(
    ...
    lp_input: list[dict] = []
    for kw in kw_perf:
        if not kw.get("avg_cpc") or kw.get("cvr") is None:
            continue
...
def _optimize_budget(items: list[dict], ctx: WorkflowContext) -> list[dict]:
    ...
    for item in items:
        ...
        #   LP side  : optimized_clicks × un-deflated raw CVR (per running day)
        #   Actual   : kw_attributed_orders / lp_scope_active_days (per running day)
        ...
        # ── active-day denominator for order_gap ─────────────────────────────
        # LP budget constraint uses daily_budget_cap ($/running-day).  Using
        # calendar days (÷30) as the denominator for sp_kw_daily_orders makes
        # order_gap overstate the reallocation gain when campaigns had significant
        # downtime (active_days << days): LP projects per-running-day performance
        # while the baseline is diluted by inactive calendar days.
        # Fix: use the max active_days across LP-scope campaigns so both sides of
        # order_gap are on the same "per running day" basis.
        ...
        sp_kw_daily_orders_per_act = round(kw_scope_orders / lp_scope_active_days, 2)

        # Calendar-day version used for lp_total_daily / inventory below.
        ...
        if days > 0:
            ...
            for cid in _all_cids:
                ...
                # budgets are already excluded from campaign_budgets (LP constraints).
                ...
                # norm_cur_orders: keyword-attributed orders (spSearchTerm, same scope as order_gap)
...
def _render_lp_reallocation_table(item: dict) -> str:
    ...
    lines = [
        "| Campaign | ACOS% | Act.d | Cur $/d | Cap $/d | LP $/d | Δ $/d | Δ ord/d | Norm Δ ord/d |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    for r in rows:
    ...
    lines.append(
        f"| {net_label} | | | | | | **{net_ds:+.1f}** | **{net_do:+.2f}** | **{norm_net_do:+.2f}** |"
    )

    # Δ ord/d uses per-campaign active days; Norm Δ ord/d uses lp_scope_active_days (_norm_d)
    # so norm_net_do should reconcile with order_gap.  Any residual reflects hidden campaigns.
    norm_residual = order_gap - norm_net_do
    if abs(norm_residual) > 0.05:
        if n_shown >= n_total:
            residual_note = (
                f"Note: Norm Δ ord/d net ({norm_net_do:+.2f}) vs order_gap ({order_gap:+.2f}) "
                f"= {norm_residual:+.2f} (rounding only; both use {_norm_d}d denominator and "
                f"spSearchTerm keyword-attributed orders as the baseline)."
            )
        else:
            n_hidden = n_total - n_shown
            residual_note = (
                f"Note: order_gap ({order_gap:+.2f}) − Norm Δ net ({norm_net_do:+.2f}) = {norm_residual:+.2f} "
                f"orders/day from {n_hidden} campaigns not shown (|Δ spend| < $1 and |Δ orders| < 0.1/day). "
                f"Δ ord/d net ({net_do:+.2f}) differs further because it uses per-campaign active days "
                f"rather than the shared {_norm_d}d denominator."
            )
        lines.append(f"\n*{residual_note}*")
...
def _build_lp_narrative(item: dict) -> dict[str, str]:
    ...
    validation_table = (
        ...
        f"The Budget Redistribution table below shows two delta columns: Δ ord/d (per-campaign "
        f"active days) and Norm Δ ord/d (same {_denom}d denominator as the Gap above — use its "
    ...
    # Spend-up actions (increase_budget) are suppressed from Top Actions when an
    ...
    _action_suffix = (
        ...
        if _spend_up_gated
        else "see Top {N} Actions below for execution steps."
    )
...
def _build_budget_interpretation(item: dict) -> dict[str, str]:
    ...
    if not starved:
        ...
        # Campaigns already classified as pause/decrease candidates — raising their budget
        # directly contradicts the action recommendation; exclude them from raise-budget advice.
        _scale_down_cids = {
            str(ca.get("campaign_id", ""))

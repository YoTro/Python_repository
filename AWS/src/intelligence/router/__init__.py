...
from ..fallback import FailureType, FallbackHandler
...
class IntelligenceRouter:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(
                f"Local provider failed to load: {e}. Some functionalities might be limited."
            )
            self.local = None
    ...
    async def route_and_execute(
        self, prompt: str, category: TaskCategory | None = None, **kwargs
    ) -> LLMResponse:
        ...
        if not category:
            ...
            logger.info(
                f"Task auto-classified as: {category} (Confidence: {confidence if confidence else 'N/A'})"
            )
        ...
        if "temperature" not in kwargs:
            ...
            logger.debug(
                f"Applied dynamic temperature: {kwargs['temperature']} for category: {category.value}"
            )
        ...
        # --- CLOUD MODEL ROUTE (with Fallback) ---
            ...
            if not self.cloud:
                response = await FallbackHandler.handle(
                    ...
                    context={"session_id": session_id, "reason": "Cloud provider not loaded"},
                )
            else:
                ...
                try:
                    if schema:
                        response = await self.cloud.generate_structured(
                    ...
                    response = LLMResponse(
                        text="FATAL: Cloud API authentication failed.",
                        provider_name="fallback",
                        model_name="auth-error",
                    )
                except Exception as e:
        ...
        trace_data = {
            ...
            "prompt": prompt
            if (is_local or kwargs.get("force_full_log"))
            else prompt[:200] + "...",
            "response": response.text
            if (is_local or kwargs.get("force_full_log"))
            else f"Length: {len(response.text)} chars (Refer to Gemini API Logs)",
        }

        await self.router_logger.log_trace(trace_data)
        return response

    async def _classify_task(self, prompt: str) -> tuple[TaskCategory, float | None]:
    ...
    def _run_heuristics(self, prompt: str) -> TaskCategory | None:
    ...
    async def record_feedback(
        self, session_id: str, ground_truth: TaskCategory, reason: str = None
    ):

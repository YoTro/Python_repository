...
class OutputParser:
    ...
    def parse_dirty_json(json_str: str, depth: int = 0) -> dict[str, Any]:
        ...
        json_match = re.search(
            r"```(?:json|JSON)?\s*(\{.*?\})\s*```", current_str, re.DOTALL | re.IGNORECASE
        )
        if json_match:
        ...
        try:
            ...
            cleaned = re.sub(
                r'("[\w ]+":\s*)"(.*?)"(?=\s*[,}\n]|$)', repair_value, current_str, flags=re.DOTALL
            )
        ...
        if depth < 1:
            ...
            if action_match:
                ...
                input_match = re.search(
                    r'"action_input":\s*(\{.*)', original_input, flags=re.DOTALL
                )
                if input_match:
                    try:
                        # The recursive call will handle balancing the captured segment
                        action_input = OutputParser.parse_dirty_json(
                            input_match.group(1), depth=depth + 1
                        )
                    except Exception:
                        pass
        ...
        if "{" in original_input and "action" in original_input:
            logger.warning(
                f"Failed to parse dirty JSON even after aggressive cleanup. String snippet: {original_input[:100]}..."
            )
        else:
            logger.debug(
                f"Input does not appear to be JSON. String snippet: {original_input[:100]}..."
            )

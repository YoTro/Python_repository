...
async def _handle_local_timeout(context: dict[str, Any]) -> LLMResponse:
    ...
    logger.warning(
        f"Local model timed out for session: {context.get('session_id')}. No cloud fallback for this error."
    )
    return LLMResponse(
...
async def _handle_cloud_unavailable(context: dict[str, Any]) -> LLMResponse:
    ...
    logger.error(
        f"Cloud API is unavailable for session: {context.get('session_id')}. Enqueuing for retry."
    )
...
class FallbackHandler:
    ...
    @classmethod
    async def handle(
        cls, failure_type: FailureType, context: dict[str, Any] | None = None
    ) -> LLMResponse:
        """

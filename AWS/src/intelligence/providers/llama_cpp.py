...
class LlamaCppProvider(BaseLLMProvider):
    ...
    def __init__(
        self, model_path: str, n_ctx: int = 32768, n_gpu_layers: int = -1
    ):  # -1 means use all GPU layers if available
        if Llama is None:
            raise ImportError(
                "llama-cpp-python is not installed. Please add it to your requirements."
            )
    ...
    async def generate_text(
        ...
        from src.intelligence.fallback import FailureType, FallbackHandler
        ...
        try:
            response = await asyncio.wait_for(
                loop.run_in_executor(
                    None,
                    functools.partial(
                        self._sync_generate_text, prompt, system_message, **filtered_kwargs
                    ),
                ),
    ...
    def _sync_generate_text(
        ...
        if len(tokens) > max_prompt_tokens:
            logger.warning(
                f"Prompt too long ({len(tokens)} tokens), truncating to {max_prompt_tokens}"
            )
        ...
        llm_kwargs = {"max_tokens": 4096, "stop": ["<|im_end|>", "<|im_start|>"], "echo": False}
        # Override with any provided kwargs
        llm_kwargs.update(kwargs)

        response = self.llm(formatted_prompt, **llm_kwargs)

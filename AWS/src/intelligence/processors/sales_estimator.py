...
class SalesRankRegressor:
    ...
    def _extract_rank(self, rank_str: str) -> int | None:
        if pd.isna(rank_str):
            return None
    ...
    def set_parameters(self, theta: float, c: float, r_squared: float | None = None):
    ...
    def fit_with_past_month_data(
        self, ranks: list[int], sales: list[int], coefficient: float = 1.2
    ) -> bool:
        """
        ...
        """
        valid_data = [
            (r, s) for r, s in zip(ranks, sales, strict=False) if r and s and r > 1 and s > 0
        ]
        if len(valid_data) < 3:
        ...
        self.is_monthly_calibrated = True
        logger.info(
            f"Calibration complete. R2: {self.r_squared:.4f}, Theta: {self.theta:.4f}, C: {self.intercept_c:.4f}"
        )
        return True

    def predict(self, rank: int) -> float:
        """Predict monthly sales based on BSR."""
        if not self.is_fitted or rank <= 1:
            return 0.0
...
class SalesEstimator:
    ...
    def __init__(self, config_path: str | None = None):
        ...
        if not config_path:
            config_path = os.path.join(
                os.path.dirname(__file__), "config", "amazon_sales_estimator.json"
            )
    ...
    def _get_regressor_for_product(self, product: Product) -> SalesRankRegressor:
        ...
        if product.category_node_id and str(product.category_node_id) in self.category_params:
            ...
            reg.set_parameters(
                theta=params["theta"], c=params["c"], r_squared=params.get("r_squared")
            )
    ...
    def calibrate_with_market_data(self, products: list[Product], node_id: str | None = None):
        ...
        if self.default_regressor.fit_with_past_month_data(ranks, sales):
            ...
            if node_id:
                ...
                self.default_regressor.__dict__.copy()  # Simplified
                self.category_params[str(node_id)] = {

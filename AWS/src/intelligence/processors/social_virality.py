...
class SocialViralityProcessor:
    ...
    def calculate_promotion_strength(
        ...
        unique_authors = len(
            {v.get("author", {}).get("uniqueId") for v in videos if v.get("author")}
        )
        ...
        if comments_data:
            ...
            comment_analysis["sentiment"]["neutral"] = round(
                1
                - (
            ...
            comment_analysis["intent"]["curiosity"] = round(
                1
                - (comment_analysis["intent"]["purchase"] + comment_analysis["intent"]["negative"]),
                2,
            )

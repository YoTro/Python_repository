...
class ProductSimilarityProcessor:
    ...
    def __init__(self, products: list[Product]):
        ...
        if "features" in self.df.columns:
            self.df["Features_Str"] = self.df["features"].apply(
    ...
    def get_similarity_matrix(self) -> np.ndarray | None:
    ...
    def cluster_products(self, n_clusters: int | None = None, method: str = "kmeans") -> list[dict]:
        """
        ...
        logger.info(
            f"Clustering complete using {method}. Found {self.df['cluster_id'].nunique()} groups."
        )

...
class ProductsNumExtractor(AmazonBaseScraper):
    ...
    def get_seller_id_from_listing(self, url: str) -> str:
        """
        ...
        # Fallback to the legacy regex method if the ID isn't found via normal DOM inspection
        # This regex looks for the dp_merchant_link pattern
    ...
    def get_products_num_by_seller(self, seller_id: str) -> int:
        """
        ...
        for attempt in range(max_retries):
            try:
                # Note: The legacy script sent JSON inside `productSearchRequestData` stringified or as dict.
                # Requests handles nested dictionaries, but `productSearchRequestData` is often expected as a JSON string
                # based on how Amazon's front-end URL encodes it.
                response = self.session.post(url, headers=headers, data=payload, timeout=15)
                response.raise_for_status()
                
                # The response is typically JSON. Using JSON parser instead of Regex is much more robust!
                try:
                    data = response.json()
                    # Example format: {"productsTotalCount": 42, "products": [...]}
                    if "productsTotalCount" in data:
                        return int(data["productsTotalCount"])
                except json.JSONDecodeError:
                    pass
                
                # Fallback to regex if JSON decoding fails (e.g., if it's wrapped in HTML)
    ...
    def get_seller_and_products_count(self, url: str) -> dict:
        """
        High-level method returning structured data for CSV saving.
        """
        seller_id = self.get_seller_id_from_listing(url)
        products_num = 0
        
        if seller_id:
            products_num = self.get_products_num_by_seller(seller_id)

...
class CartStockExtractor(AmazonBaseScraper):
    ...
    def get_stock(self, asin: str, host: str = "https://www.amazon.com") -> dict:
        """
        ...
        # 2. Add to cart
        if not self._add_to_cart(asin, payload, host):
            return result
            
        # 3. View cart to get tokens for quantity update
        cart_data = self._get_cart_view(host)
        ...
        update_result = self._update_quantity(asin, cart_data, host)
        result["Stock"] = update_result["value"]
        result["StockStatus"] = update_result["status"]
        
        # 5. Cleanup: Delete from cart
        self._delete_from_cart(asin, result["Stock"], cart_data, host)
    ...
    def _get_dynamic_form_parameters(self, html: str, asin: str) -> dict:
        ...
        logger.debug(f"Extracted payload tokens for {asin}: {[k for k in payload.keys() if 'token' in k or 'session' in k]}")
        
        # Fallback and common overrides
        if not payload.get('offerListingID'):
            match = re.search(r'name="offerListingID" value="(.*?)"', html)
            if match: payload['offerListingID'] = match.group(1)

        if 'session-id' not in payload:
            match = re.search(r'session-id=(.*?);', html)
            if match: payload['session-id'] = match.group(1)
    ...
    def _add_to_cart(self, asin: str, payload: dict, host: str) -> bool:
        ...
        try:
            ...
            res = self.session.post(url, data=encoded_data, headers=headers, timeout=15)
    ...
    def _get_cart_view(self, host: str) -> dict:
        ...
        try:
            ...
            data = {
                "price": self._regex_first(r'data-price="(.*?)"', html) or self._regex_first(r'sc-product-price.*?>(.*?)</span>', html),
    ...
    def _update_quantity(self, asin: str, cart_data: dict, host: str) -> dict:
        """
            ...
            if res.status_code == 200:
                try:
                    ...
                    if 'features' in data and 'nav-cart' in data['features']:
                        ...
                        # If quantity is capped and significantly less than 999, it's likely a limit/stock issue 
                        # but we couldn't find the specific message. 
                        # However, without a message, we label as 'Actual' or 'Unknown'.
                        return {"value": qty, "status": "Actual" if qty < 999 else "Actual"}
                except Exception as e:
    ...
    def _delete_from_cart(self, asin: str, stock: int, cart_data: dict, host: str):
        ...
        try:
            ...
            encoded_data = urlencode(payload)
            self.session.post(url, data=encoded_data, headers=headers, timeout=15)
        except Exception as e:

...
async def handle_export_csv(name: str, arguments: dict) -> list[TextContent]:
    ...
    if not items:
        return [
            TextContent(
                type="text", text=json.dumps({"success": False, "error": "No items provided"})
            )
        ]
    ...
    try:
        ...
        try:
            ...
            return [
                TextContent(type="text", text=json.dumps({"success": True, "url": url}, indent=2))
            ]
        ...
        inputSchema={
            ...
            "properties": {
                "items": {
                    "type": "array",
                    "items": {"type": "object"},
                    "description": "List of flat dicts to export.",
                },
                "filename": {
                    "type": "string",
                    "description": "Output filename, e.g. 'keywords.csv'.",
                },
            },
            "required": ["items"],
        },

...
class XiyouZhaociAuth:
    ...
    def _save_auth_state(self, ticket: str):
        try:
            state = {"ticket": ticket, "created_at": time.time(), "tenant_id": self.tenant_id}
    ...
    def check_wechat_login(self, ticket: str = None) -> dict:
        ...
        if ticket:
            ...
            if elapsed > 115:
                ...
                return {
                    "status": "EXPIRED",
                    "msg": "QR code has expired. Please request a new one.",
                }
    ...
    def send_sms_code(self, phone_num: str = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to parse JSON. Status: {response.status_code}, Raw response: {response.text}"
                )
                return False

            if (
                data.get("code") == 200
                or data.get("code") == "success"

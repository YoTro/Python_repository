...
async def handle_market_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    if name == "sellersprite_competing_lookup":
        ...
        requested_ym = (
            int(month_name.replace("bsr_sales_monthly_", ""))
            if month_name.startswith("bsr_sales_monthly_")
            else 0
        )
            ...
            if not m:
                return [
                    TextContent(
                        type="text",
                        text=json.dumps(
                            {"error": f"Could not extract node ID from URL: {amazon_url}"},
                            ensure_ascii=False,
                        ),
                    )
                ]
        ...
        if not node_id_paths:
            return [
                TextContent(
                    type="text",
                    text=json.dumps(
                        {"error": "Either amazon_url or node_id_paths is required"},
                        ensure_ascii=False,
                    ),
                )
            ]
        ...
        slim_items = [
            {k: v for k, v in item.items() if k in _KEEP} for item in (raw.get("items") or [])
        ]
        result = {
            "snapshot": month_name,
            "today": now.strftime("%Y-%m-%d"),
        ...
        if raw_table and _re.fullmatch(r"\d{6}", raw_table):
            ...
            table = (
                f"bsr_sales_monthly_{int(m2.group(1)):04d}{int(m2.group(2)):02d}"
                if m2
                else raw_table
            )
        elif raw_table:
    ...
    # ── Xiyouzhaoci NEW WeChat QR Auth Tools ─────────────────────────────
        ...
        if "url" in qr_data:
            ...
            signal = {
                ...
                "data": {"url": qr_data["url"], "expires_in": qr_data.get("expires_in", 120)},
                "context": {
                    "tenant_id": tenant_id,
    ...
    # ── Xiyouzhaoci Legacy SMS Auth Tools ───────────────────────────────
        ...
        if not phone:
            return [
                TextContent(
                    type="text",
                    text=json.dumps({"status": "error", "error": "Phone number not provided."}),
                )
            ]
                    ...
                    text=json.dumps(
                        {
                            "status": "sms_sent",
                            "phone": masked,
                            "message": f"SMS sent to {masked}. Call xiyou_verify_sms.",
                        }
                    ),
                )
            ]
        return [
            TextContent(
                type="text",
                text=json.dumps({"status": "sms_failed", "error": "Failed to send SMS."}),
            )
        ]
        ...
        return [
            TextContent(
                type="text",
                text=json.dumps({"status": "auth_failed", "error": "SMS verification failed."}),
            )
        ]
        ...
        if name in tool_map:
            try:
                ...
                if isinstance(result, str) and os.path.exists(result):
                    return [
                        TextContent(
                            type="text", text=json.dumps({"status": "success", "file_path": result})
                        )
                    ]
                ...
                return [
                    TextContent(
                        type="text", text=json.dumps({"status": "AUTH_REQUIRED", "message": str(e)})
                    )
                ]
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "amazon_url": {
                    "type": "string",
                    "description": "Amazon BSR category URL, e.g. 'https://www.amazon.com/gp/bestsellers/industrial/8297518011/'",
                },
                "market": {
                    "type": "string",
                    "default": "US",
                    "description": "Marketplace code (US, DE, JP, …)",
                },
                "month_name": {
                    "type": "string",
                    "description": "BSR snapshot table name, e.g. 'bsr_sales_monthly_202602'. Defaults to latest published snapshot.",
                },
                "node_id_paths": {
                    "type": "array",
                    "items": {"type": "string"},
                ...
                "order": {
                    "type": "object",
                    "description": 'Sort spec, e.g. {"field": "bsr_rank", "desc": false}',
                },
                "symbol_flag": {
                    "type": "boolean",
                    "default": True,
                    "description": "Include brand symbol filter",
                },
                "low_price": {
                    "type": "string",
                    "default": "N",
                    "description": "Low-price filter flag (Y/N)",
                },
            },
            "required": [],
        },
        ...
        inputSchema={
            ...
            "properties": {
                "market_id": {
                    "type": "integer",
                    "default": 1,
                    "description": "Numeric market ID (1=US, 6=DE, …)",
                },
                "month_name": {
                    "type": "string",
                    "description": "BSR snapshot month, e.g. '2026-02' or '202602'. Defaults to previous month.",
                },
                "query": {
                    "type": "string",
                    "description": "Numeric node ID or category keyword to search",
                },
            },
            "required": ["query"],
        },
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "table": {
                    "type": "string",
                    "description": "BSR snapshot table name, e.g. 'bsr_sales_monthly_202509'",
                },
                "node_id_path": {
                    "type": "string",
                    "description": "Colon-joined node path, e.g. '16310091:8297370011'",
                },
            },
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "node_id_path": {
                    "type": "string",
                    "description": "Top-level category node ID, e.g. '1055398' for Home & Kitchen",
                },
                "market_id": {
                    "type": "integer",
                    "default": 1,
                ...
                "month_name": {
                    "type": "string",
                    "default": "bsr_sales_nearly",
                    "description": "Snapshot name, e.g. 'bsr_sales_nearly' or 'bsr_sales_monthly_202602'",
                },
                "page": {
                    "type": "integer",
                    "default": 1,
                    "description": "Page number (1-based, 10 rows per page)",
                },
                "size": {
                    "type": "integer",
                    "default": 20,
                    "description": "Rows requested per page (server returns ~10 regardless)",
                },
            },
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "phone": {
                    "type": "string",
                    "description": "Phone number (defaults to XIYOUZHAOCI_PHONE env var)",
                },
            },
        },
    ),
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "keyword": {
                    "type": "string",
                    "description": "Search term to analyze (e.g. 'wireless charger')",
                },
                "country": {
                    "type": "string",
                    "default": "US",
                ...
                "output_dir": {
                    "type": "string",
                    "default": "data",
                    "description": "Local directory for the downloaded xlsx",
                },
            },
            "required": ["keyword"],
        },
        ...
        inputSchema={
            ...
            "properties": {
                "asins": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "List of ASINs to compare (max 20)",
                },
                "country": {
                    "type": "string",
                    "default": "US",
                ...
                "period": {
                    "type": "string",
                    "default": "last7days",
                    "description": "Time period: 'last7days' or 'last30days'",
                },
                "output_dir": {
                    "type": "string",
                    "default": "data",
        ...
        inputSchema={
            ...
            "properties": {
                "search_terms": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Search terms to query (e.g. ['wireless charger', 'usb c hub'])",
                },
                "country": {
                    "type": "string",
                    "default": "US",
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Root query string (e.g. 'iphone 15 case')",
                },
                "country": {
                    "type": "string",
                    "default": "US",
                ...
                "field": {
                    "type": "string",
                    "default": "week",
                    "description": "Time granularity: 'week' or 'month'",
                },
                "rank_pattern": {
                    "type": "string",
                    "default": "aba",
                    "description": "Ranking source pattern (default: 'aba')",
                },
            },
            "required": ["query"],
        },
        ...
        inputSchema={
            ...
            "properties": {
                "asins": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "List of ASINs to query",
                },
                "country": {
                    "type": "string",
                    "default": "US",
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "start_date": {
                    "type": "string",
                    "description": "Start date (YYYY-MM-DD). Earliest: 2023-02-01",
                },
                "end_date": {
                    "type": "string",
                    "description": "End date (YYYY-MM-DD). Max span: 25 months",
                },
            },
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "end_date": {
                    "type": "string",
                    "description": "End date YYYY-MM-DD (e.g. '2026-04-21')",
                },
                "page": {"type": "integer", "default": 1, "description": "Page number (1-based)"},
                "page_size": {
                    "type": "integer",
                    "default": 50,
                    "description": "Keywords per page (max 50)",
                },
            },
            "required": ["asin", "start_date", "end_date"],
        },
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                "search_term": {
                    "type": "string",
                    "description": "The keyword to query (e.g. 'nebulizer')",
                },
                "country": {
                    "type": "string",
                    "default": "US",
                ...
                "weeks": {
                    "type": "integer",
                    "description": "Number of recent weeks to return (default: API decides, typically 52)",
                },
                "week_interval": {
                    "type": "array",
                    "items": {"type": "string"},
...
_MARKET_META = {
    "sellersprite_competing_lookup": (
        "DATA",
        "paginated BSR competitor list with monthly sales trends",
    ),
    "sellersprite_resolve_node_path": (
        "DATA",
        "full nodeIdPath resolved from a bare Amazon BSR node ID via nodeLabelPath search",
    ),
    ...
    "xiyou_keyword_analysis": (
        "DATA",
        "xlsx file with ASINs, traffic data, ranking trends (third-party)",
    ),
    "xiyou_asin_lookup": (
        "DATA",
        "xlsx file with reverse-lookup keywords for an ASIN (third-party)",
    ),
    "xiyou_asin_compare_keywords": (
        "DATA",
        "xlsx file with multi-ASIN keyword comparison data (third-party)",
    ),
    "xiyou_get_aba_top_asins": (
        "DATA",
        "JSON containing top ASINs and metrics for specified search terms",
    ),
    "xiyou_get_search_terms_ranking": (
        "DATA",
        "JSON containing search frequency ranks and trends for variations of a query",
    ),
    "xiyou_get_traffic_scores": (
        "DATA",
        "JSON containing traffic scores, ad ratio, and growth for ASINs",
    ),
    "xiyou_get_asin_daily_trends": (
        "DATA",
        "JSON containing daily historical trends for price and ratings",
    ),
    "xiyou_get_search_term_trends": (
        "DATA",
        "JSON containing weekly ABA SFR history for a keyword (52-week seasonality signal)",
    ),
    "xiyou_get_asin_keywords": (
        "DATA",
        "JSON list of keywords driving traffic to an ASIN, with topAsins per keyword and traffic ratio breakdown",
    ),
}

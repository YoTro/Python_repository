...
class AsinExtractor(AmazonBaseScraper):
    """
    Extractor to find the sales ASINs in Amazon search results.
    """

    async def extract_asins_from_search(self, keyword: str, page: int = 1) -> list:
        """
        ...
        matches = re.findall(
            r"dp\/(B[A-Z0-9]{9}).*?a-row a-size-base\"><span class=\"a-size-base a-color-secondary\">(.*?)<\/span>",
            html,
        )
    ...
    async def get_asins(self, keyword: str, page: int = 1) -> list:
        """
        Higher level function returning structured data for CSV saving.
        """
        asins = await self.extract_asins_from_search(keyword, page)

        results = []
        for asin in asins:
            results.append({"Keyword": keyword, "Page": page, "ASIN": asin})

...
class KeywordsRankExtractor(AmazonBaseScraper):
    ...
    async def get_keyword_results_count(self, keyword: str) -> int:
        ...
        match_legacy = re.search(
            r"a-section a-spacing-small a-spacing-top-small\">\s+<span>.*?over (.*?)results for",
            html,
        )
        if match_legacy:
            try:
    ...
    async def get_asin_ranks_for_keyword(
        self, keyword: str, target_asins: list, max_pages: int = 3
    ) -> list:
        """
            ...
            if not search_results:
                ...
                for asin in target_asins:
                    if asin in found_asins:
                        continue
                    match = re.search(rf'data-asin="{asin}" data-index="(\d+)"', html)
                    if match:
                        rank_val = int(match.group(1))
                        all_ranks.append(
                            {"ASIN": asin, "Keyword": keyword, "Page": page, "Rank": rank_val}
                        )
                ...
                for result in search_results:
                    ...
                    if asin and asin in target_asins and asin not in found_asins:
                        try:
                            ...
                            all_ranks.append(
                                {"ASIN": asin, "Keyword": keyword, "Page": page, "Rank": index}
                            )
                            found_asins.add(asin)
                        except ValueError:
                            pass

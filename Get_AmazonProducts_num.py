...
def post_products(sellerID):
    ...
    "gzip, deflate, br",
    "Referer":
    "https://www.amazon.com/sp?_encoding=UTF8&asin=&isAmazonFulfilled=1&isCBA=&marketplaceID=ATVPDKIKX0DER&orderID=&seller={0}&tab=&vasStoreID=".format(sellerID),#匹配相应sellerID
    "Content-Type":
    "application/x-www-form-urlencoded",
    "Content-Length":	
    "313",
    "X-Requested-With":
    "XMLHttpRequest",#异步传输方式
    "Cookie":
    ...
    "productSearchRequestData":{"marketplace":"ATVPDKIKX0DER","seller":"{0}".format(sellerID),"url":"/sp/ajax/products","pageSize":12,"searchKeyword":"","extraRestrictions":{},"pageNumber":1}}#请求主体
    r=requests.post(post_url,data=data,headers=headers)
    return r.content
def requests_url(url):
    '''获取listing网页内容（主要用于匹配sellerID）'''
    headers={"Host":	
"www.amazon.com",
"User-Agent":
...
def main():
    ...
    for i in xrange(rows-1):
        try:
            if FBA[i+1]=='FBA' or 'MCH': #判断是否为FBA或是自配送运输方式（因为只有该运输方式和第三方运输可以查询店铺产品数量
                ...
                print sellerID[0]
                if sellerID:
                    post=post_products(sellerID[0])#传入参数发出post请求返回json
                    products_num=re.findall(r'\"productsTotalCount\":(\d+)',post)#匹配上架商品数量
                    print products_num
                    table1.write(i+1,cols,products_num)
                    table1.write(i+1,cols+1,sellerID[0])
                else:
                    print 'None'
             if FBA[i+1]=='AMZ':#如果是亚马逊官方卖家跳过
                i+=1
        except Exception,e:
            print str(e)
